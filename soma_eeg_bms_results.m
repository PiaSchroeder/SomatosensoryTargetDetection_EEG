...
glm_dir = 'bayesglm';
trg_dir = fullfile(bms_dir,'results');
...
%% Load data
ERP_file = fullfile(erp_dir, ['ERPs_' cond '.mat']);
load(ERP_file)
...
beta_file = fullfile(bms_dir, 'betas.mat');
...
if exist(beta_file,'file') && ~recompute_betas
    ...
    for s = 1:nSubjects
        ...
        sj_glm_dir = fullfile(data_dir, SJs{s}, glm_dir);
...
erp_cols = parula(ncond+1);
erp_cols = erp_cols(1:ncond,:);
...
%-- 2. Plot XP time courses ---------------------------------------------
    ...
    set(gca,'YDir','reverse','ytick',1:nModels,'YTickLabel',{},'xtick',xticks,'XTickLabel',{},'tickdir','out');%,'xtick',-.5:.25:1.75,'ytick',6:2:48);
    set(gca,...
        'YDir','reverse',...
        'ytick',1:nModels,...
        'YTickLabel',{},...
...
%-- 3. plot betas -------------------------------------------------------
    ...
    img_name = [chan '_BMS_results.tiff'];
    saveas(f, fullfile(trg_dir,img_name));
